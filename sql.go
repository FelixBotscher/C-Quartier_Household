...
func dbPutPowerdifferenceToChangesTable(val float64, usr Usr) error {
	db := dbConnection()
	defer db.Close()

	_, err := db.Exec("INSERT INTO changes(userid,wchange) values (UUID_TO_BIN(?, true),?);", usr.Uuid, val)
	return err
}
